...
ci.upper = rep(0,5)
for (i in 1:5) {
  ci.lower <- theta1$par[i]-1.96*se[i] # lower bound for each parameter
  ci.upper = theta1$par[i]+1.96*se[i] # upper bound for each parameter
}
...
# comment: since log(N(t))=n0+sum(beta*t^i) is a linear transformation (sum) of the parameters, we have the 95% confidence interval for log(N(t)) is the sum of 95% confidence intervals for each parameter. 
